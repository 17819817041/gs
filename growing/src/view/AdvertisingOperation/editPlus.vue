									...
									<div class="textarea_wrap_item float" v-for="(item,i) in ruleForm.imageList" :key="i">
										<div class="imageList_wrap">
											...
											<div class="textarea_wrap_item_child ju al cursor">
												...
												style="height: 100%;object-fit:cover;width: 100%" :src="item.url" alt="">
...
import { genPackageOrder, getListByTypeId, getbyId, getuploadtoken, getGuangGaoDetailsById, updatePackageGuangGao } from "@/axios/request.js"
export default {
    data() {
        return {
			uploadHeaders: {
    ...
    methods: {
		getGuangGaoDetailsById () {
            ...
            getGuangGaoDetailsById(data).then(res => {
                ...
                if (res.data.rtnCode == 200) {
					...
					this.getbyId(obj.packageId)
                    this.ruleForm.startDate = new Date(obj.startTime)
					this.ruleForm.endDate = new Date(obj.endTime)
					if ( obj.guangGaoContentDtos) {
						this.ruleForm.cmediaType = obj.guangGaoContentDtos[0].fileType
						if (obj.guangGaoContentDtos[0].fileType == 1) {
						...
						obj.guangGaoContentDtos.forEach(item => {
							item.url = item.fileUrl
							item.size = item.fileSize
							item.name = item.fileName
							item.video = item.filePlayTime
						})
						this.ruleForm.imageList = obj.guangGaoContentDtos
					}
		...
		updatePackageGuangGao () {
			...
			updatePackageGuangGao(data1).then(res => {
				this.loading = false
				console.log(res)
				if (res.data.rtnCode == 200) {
					this.submit = false
