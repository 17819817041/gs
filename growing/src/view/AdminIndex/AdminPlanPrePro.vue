<template>
    <div class="AdvertisingAdd AdvertisingAdd1">
		<div class="AdvertisingOperation_back mg al">
            <img class="cursor" style="padding: 0 15px;" src="@/assets/img/back_arrow.png" alt="" @click="goBack">廣告媒體內容
        </div>
			...
			<div :class="['content mg bar',{ heigh: !submit }]">
				...
				<div class="noBar" style="height: calc(100% - 0px); overflow:auto" v-show="submit">
					<div class="basicsMsg theme" v-show="submit">
						...
						<el-form :model="ruleForm" :label-position="$i18n.locale == 'zh-CN'? 'left' : 'top'" ref="ruleForm" 
						:label-width="$i18n.locale == 'zh-CN'? '125px': '165px'" class="demo-ruleForm">
								...
								<div :class="['textarea_wrap clear', { content_down: $i18n.locale == 'zh-CN' }]">
									<div class="textarea_wrap_item float cursor" v-for="(item,i) in imageList" :key="i">
										<div class="imageList_wrap">
											<div class="textarea_wrap_item_child ju al" v-if="item.fileType == 1" @click="imgPreview(item.url)">
												<img style="height: 100%;" :src="item.url" alt="">
											</div>
											<!-- <div class="textarea_wrap_item_child ju al" v-else-if="item.fileType == 3" @click="imgPreview(item.url)">
												<img style="height: 100%;" :src="item.url" alt="">
											</div> -->
											<div class="textarea_wrap_item_child ju al cursor" 
											 v-else-if="item.fileType == 3" @click="previewVideo(item)">
							...
							<el-form-item :label="$t('lang.amt')">
								...
								<div :class="['clear', { content_down2: $i18n.locale == 'zh-CN' }]"
								v-show="radio1 == '1'" style="margin-top: 10px;">
										...
										<div style="color: #B0B0B0;font-size: 13px;" 
											...
											<div v-if="item.guangGaoTimeMinDtos.length != 0">
												...
												<span style="margin-left: 3px;">
													({{item.guangGaoTimeMinDtos[0].timeMin}}{{$t('lang.minute')}})
												</span>
											</div>
										</div>
									</div>
...
import { adminguangGaoById } from "@/axios/request.js"
export default {
    data() {
        return {
			showVideo: false,
    ...
    methods: {
		...
		guangGaoById () {
			...
			adminguangGaoById(data).then(res => {
				console.log(res)
				if (res.data.rtnCode == 200) {
					...
					item.guangGaoContentDto.forEach(key => {
						if (key.fileType == 3) {
